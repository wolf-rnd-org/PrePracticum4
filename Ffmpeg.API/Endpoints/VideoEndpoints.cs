    ...
    public static class VideoEndpoints
    {
            ...
            [FromForm] BorderDto dto)
        {
            ...
            try
            {
                ...
                string videoFileName = await fileService.SaveUploadedFileAsync(dto.VideoFile);
                string extension = Path.GetExtension(dto.VideoFile.FileName);
                string outputFileName = dto.OutputFileName ?? await fileService.GenerateUniqueFileNameAsync(extension);
                ...
                try
                {
                    ...
                    var result = await command.ExecuteAsync(new BorderModel
                    {
                        InputFile = videoFileName,
                        OutputFile = outputFileName,
                        BorderColor = string.IsNullOrWhiteSpace(dto.BorderColor) ? "black" : dto.BorderColor,
                        BorderThickness = 20,
                        VideoCodec = "libx264"
                    });
                    if (!result.IsSuccess)
                ...
                {
                    logger.LogError(ex, "Error processing AddBorder request");
                    _ = fileService.CleanupTempFilesAsync(filesToCleanup);
                    throw;
                }
            }
