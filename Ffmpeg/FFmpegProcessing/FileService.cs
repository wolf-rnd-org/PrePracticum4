    ...
    public class FileService : IFileService
    {
        ...
        public async Task<string> SaveUploadedFileAsync(IFormFile file)
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Saved file {FileName} ({Size} bytes)", fileName, file.Length);
                return fileName;
            }
            catch (Exception ex)
            {
        ...
        public async Task<byte[]> GetOutputFileAsync(string fileName)
        {
            ...
            {
                _logger.LogError(ex, "Error reading output file {FileName}", fileName);
        ...
        public async Task CleanupTempFilesAsync(IEnumerable<string> fileNames)
        {
            foreach (var fileName in fileNames)
            {
                ...
                {
                    _logger.LogWarning(ex, "Error cleaning up file {FileName}", fileName);
